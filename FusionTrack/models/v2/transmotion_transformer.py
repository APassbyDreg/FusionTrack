...
class TransMotionTransformer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # ------------------- merge track reference points ------------------- #
        reference_points = torch.cat([reference_points, track_reference_points], dim=1)
        ...
        # ------------------------ merge track embeds ------------------------ #
        target = torch.cat([target, track_embeds], dim=1)
        ...
        if self.combine_det_track or track_embeds.shape[1] == 0:
            ...
            return (
                torch.cat([target_unact, track_embeds], dim=1),
                torch.cat([det_states, track_states], dim=2),
                torch.cat([init_reference_out, track_reference_points], dim=1),
