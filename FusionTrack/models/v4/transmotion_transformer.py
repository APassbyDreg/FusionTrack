        ...
        **kwargs,
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            if self.bbox_embed is not None:
                if self.track_bbox_embed is not None and output.shape[1] > self.num_det_queries:
                    out_detect, out_track = torch.split(output, [self.num_det_queries, output.shape[1] - self.num_det_queries], dim=1)
                    tmp_detect = self.bbox_embed[layer_idx](out_detect)
                    tmp_track = self.track_bbox_embed[layer_idx](out_track)
                    tmp = torch.cat([tmp_detect, tmp_track], dim=1)
                else:
