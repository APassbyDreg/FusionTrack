...
class TransMotionTransformerDecoderLayer(nn.Module):
    # a modified version of BaseTransformerLayer in detrex
    ...
    def __init__(
        ...
        self.track_ffns = nn.ModuleList()
        num_ffns = operation_order.count("ffn")
        for _ in range(num_ffns):
            self.ffns.append(copy.deepcopy(ffn))        # use the same name to align with checkpoint
            self.track_ffns.append(copy.deepcopy(ffn))  # NEW: track ffn
        ...
        **kwargs contains the specific arguments of attentions.
        ...
        for layer in self.operation_order:
            if layer == "self_attn":
                ...
                assert isinstance(attn_mask, torch.Tensor) or attn_mask is None
                if self.mask_track2det and query.shape[1] > self.num_det_queries:
                    # one indicates not allowed to attend
                    mask = torch.zeros((query.shape[1], query.shape[1]), device=query.device, dtype=torch.bool)
                    mask[:self.num_det_queries, self.num_det_queries:] = True
                    if attn_masks[attn_index] is None:
                        attn_mask = mask
                    elif attn_mask[attn_index].dtype == torch.bool:
                        attn_mask = attn_masks[attn_index] | mask
                    else:
                ...
                # ------------------ following are new code ------------------ #
                # we assume the attn is batch first
                if query.shape[1] == self.num_det_queries:
                    query = self.ffns[ffn_index](query, identity if self.pre_norm else None)
                else:
                    T = query.shape[1] - self.num_det_queries
                    det_query, track_query = torch.split(query, [self.num_det_queries, T], dim=1)
                    det_identity, track_identity = torch.split(identity, [self.num_det_queries, T], dim=1)
                    det_query = self.ffns[ffn_index](det_query, det_identity if self.pre_norm else None)
                    track_query = self.track_ffns[ffn_index](track_query, track_identity if self.pre_norm else None)
                    query = torch.cat([det_query, track_query], dim=1)
