...
class LoadAnnotations:
    ...
    def _load_instances(self, results):
        if 'instance_ids' in results['ann_info']:
            results['gt_instance_ids'] = results['ann_info']['instance_ids'].copy()
        else:
            # for pseudo video
            results['gt_instance_ids'] = np.arange(len(results['gt_labels']))
        return results
    ...
    def __call__(self, results):
        ...
        if self.with_instance:
            results = self._load_instances(results)
        if self.with_mask:
...
class FilterAnnotations:
    ...
    def __call__(self, results):
        ...
        keys = ('gt_bboxes', 'gt_labels', 'gt_masks', 'gt_instance_ids')
        for key in keys:
            if key in results:
