...
class CosineSimilarityCost:
    ...
    def __call__(self, feat_pred, feat_target):
        """
        Args:
            feat_pred (Tensor):  Shape (num_query, n).
            feat_traget (Tensor): Shape (num_gt, n).
        Returns:
            torch.Tensor: Lk_cost value with weight
        """
        feat_pred = feat_pred / feat_pred.norm(dim=-1, keepdim=True)
        feat_target = feat_target / feat_target.norm(dim=-1, keepdim=True)
        cos_sim = feat_pred @ feat_target.T
        cost = 0.5 - cos_sim * 0.5
...
class LpNormCost:
    ...
    Args:
        p (int, optional): norm type
        weight (int | float, optional): loss_weight
    """

    def __init__(self, p=1, weight=1.0):
        self.p = p
        self.weight = weight
    ...
    def __call__(self, pred, target):
        """
        Args:
            pred (Tensor):  Shape (num_query, n).
            traget (Tensor): Shape (num_gt, n).
        Returns:
            torch.Tensor: Lp_cost value with weight
        """
        P, N = pred.shape
        T = target.shape[0]
        cost = torch.cdist(pred, target, p=self.p) / N
        return cost * self.weight
...
class ClassPredCost:
    ...
    def __call__(self, pred, target):
        ...
        P, N = pred.shape
        T = target.shape[0]
        cost = torch.zeros(P, T, N).to(pred.device)
        for n in range(N):
            cost[:, :, n] = torch.cdist(pred[:, [n]], target[:, [n]], p=self.p)
            cost[:, :, n] *= target[:, n].unsqueeze(0)
            cost[:, :, n] *= pred[:, n].unsqueeze(1)
...
class IoUCost:
    ...
    def __call__(self, bboxes, gt_bboxes):
        ...
        iou_cost = -overlaps * 0.5 + 0.5 # remap to [0, 1]
