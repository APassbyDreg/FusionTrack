...
@MOTION.register_module()
class KalmanFilterCXCYWH(object):
    """A simple Kalman filter for tracking bounding boxes in image space.
    ...
    def initiate(self, measurement):
        ...
        Args:
            measurement (ndarray):  Bounding box coordinates (cx, cy, w, h) with
        ...
        diag = np.sqrt(np.dot(measurement[2:], measurement[2:]))
        
        std = [
            2 * self._std_weight_position * diag,
            2 * self._std_weight_position * diag,
            2 * self._std_weight_position * diag,
            2 * self._std_weight_position * diag,
            10 * self._std_weight_velocity * diag,
            10 * self._std_weight_velocity * diag,
            10 * self._std_weight_velocity * diag,
            10 * self._std_weight_velocity * diag,
        ]
    ...
    def predict(self, mean, covariance):
        ...
        Returns:
            (ndarray, ndarray): Returns the mean vector and covariance
                ...
                initialized to 0 mean.
        """
        diag = np.sqrt(np.dot(mean[2:], mean[2:]))
        std_pos = [
            self._std_weight_position * diag,
            self._std_weight_position * diag,
            self._std_weight_position * diag,
            self._std_weight_position * diag,
        ]
        std_vel = [
            self._std_weight_velocity * diag,
            self._std_weight_velocity * diag,
            self._std_weight_velocity * diag,
            self._std_weight_velocity * diag,
        ]
    ...
    def gating_distance(self,
        ...
        Args:
            ...
            covariance (ndarray): Covariance of the state distribution (8x8
                dimensional).
            measurements (ndarray): An Nx4 dimensional matrix of N
                measurements, each in format (x, y, w, h) where (x, y) is the
                bounding box center position, w the width, and h the
                height.
            only_position (bool, optional): If True, distance computation is
                done with respect to the bounding box center position only.
                Defaults to False.
